...
// ── Alive animation timings ────────────────────────────────────────────────────

// Blink: quick close-open every ~4 s (asymmetric keyframe times)
const BLINK = {
  animate: { ry: [7, 7, 0.25, 7, 7] as number[] },
  transition: {
    duration: 5.0,
    repeat: Infinity as number,
    times: [0, 0.74, 0.80, 0.86, 1.0],
    ease: "easeInOut" as const,
  },
};

// Second eye blinks slightly offset so they don't look robotic
const BLINK2 = {
  animate: { ry: [7, 7, 0.25, 7, 7] as number[] },
  transition: {
    duration: 5.0,
...
// Breathing: torso group gently expands then contracts
const BREATH = {
  animate: { scaleY: [1, 1.022, 1] as number[] },
  transition: {
    duration: 4.4,
    repeat: Infinity as number,
    ease: "easeInOut" as const,
...
// Gem pulse: chest crystal glows larger / smaller
const GEM = {
  animate: { scale: [1, 1.18, 1] as number[] },
  transition: {
    duration: 3.2,
    repeat: Infinity as number,
    ease: "easeInOut" as const,
...
// Antenna: tip glow breathes in/out
const ANT = {
  animate: { opacity: [0.88, 1, 0.48, 1, 0.88] as number[], scale: [1, 1.22, 0.86, 1.12, 1] as number[] },
  transition: {
    duration: 2.4,
    repeat: Infinity as number,
    ease: "easeInOut" as const,
...
function FullBodyNovaRobot({ glowing = false }: { glowing?: boolean }) {
  return (
    <svg viewBox="0 0 80 132" fill="none" overflow="visible"
      ...
      {/* Pulsing antenna glow — alive */}
      <motion.circle cx="40" cy="4" r="5"   fill={C} opacity="0.1" />
      <motion.circle
        cx="40" cy="4" r="3.2" fill={C}
        animate={ANT.animate}
        transition={ANT.transition}
      ...
      {/* ─ Left eye ─ — BLINKING lid */}
      <motion.ellipse
        cx="32" cy="25.5" rx="6.5"
      ...
      {/* ─ Right eye ─ — BLINKING lid (slight delay) */}
      <motion.ellipse
        cx="48" cy="25.5" rx="6.5"
        fill={`${C}16`} stroke={C} strokeWidth="1.2"
        animate={BLINK2.animate}
        transition={BLINK2.transition}
      />
      ...
      <motion.g
        ...
        transition={BREATH.transition}
        style={{ transformOrigin: "40px 68px" }}
      >
        {/* Hourglass feminine silhouette */}
        ...
        {/* Chest power gem — pulsing alive */}
        <circle cx="40" cy="66" r="7.5" fill={`${P}28`} stroke={P} strokeWidth="1.2" />
        <motion.circle
          cx="40" cy="66" r="4.5"
          fill="url(#nv-gem)" opacity="0.88"
          animate={GEM.animate}
          transition={GEM.transition}
...
interface SparkleProps { left: string; top: string; delay: number; color: string; size?: number }
    ...
    <AnimatePresence>
      {visible && !isOpen && (
        ...
        >
          ...
          <Sparkle left="-16%"  top="78%"  delay={0.5}  color={P}  size={5} />
          <Sparkle left="54%"   top="-8%"  delay={2.3}  color={C}  size={4} />

          {/* Robot — bobbing float + 3-D mouse-follow tilt */}
          <motion.div
          ...
          {/* Ground shadow — synced with bob */}
          <motion.div
            className="w-14 h-3.5 rounded-full -mt-1"
            style={{
              background: "radial-gradient(ellipse, rgba(0,245,255,0.45) 0%, transparent 70%)",
            }}
