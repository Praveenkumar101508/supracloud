 ...
 * Run with: npx ts-node --project tsconfig.json qa-output/link-checker.ts
 */
...
async function checkInternalRoutes(): Promise<LinkResult[]> {
  ...
  for (const route of INTERNAL_ROUTES) {
    ...
    try {
      ...
      results.push({ url: route, type: "internal", status: "TIMEOUT", ok: false, note: err.message?.substring(0, 100) });
...
async function checkExternalLinks(): Promise<LinkResult[]> {
  ...
  for (const url of EXTERNAL_LINKS) {
    ...
    try {
      ...
      results.push({ url, type: "external", status: "ERROR", ok: false, note: err.message?.substring(0, 100) });
...
async function run() {
  ...
  md += `| External links checked | ${external.length} |\n`;
  ...
  if (broken.length > 0) {
    ...
    for (const r of broken) {
      md += `- **${r.url}** — ${r.status}${r.note ? ` (${r.note})` : ""}\n`;
    }
  } else {
    md += `\n## Result\n\nAll links returned non-error status codes. ✅\n`;
  }

  md += `\n---\n*Generated ${new Date().toISOString()}*\n`;

  const outPath = path.resolve(__dirname, "broken-links-report.md");
